...
class Routes(
  ...
  def documentation = List(
    ...
    ("""GET""", this.prefix + (if(this.prefix.endsWith("/")) "" else "/") + """cancelOrder/""" + "$" + """orderId<[^/]+>""", """controllers.ShoppingCtrl.cancelOrder(orderId:Long)"""),
  ...
  def routes: PartialFunction[RequestHeader, Handler] = {
    ...
    case controllers_ShoppingCtrl_cancelOrder14_route(params@_) =>
      call(params.fromPath[Long]("orderId", None)) { (orderId) =>
        controllers_ShoppingCtrl_cancelOrder14_invoker.call(ShoppingCtrl_1.cancelOrder(orderId))
      }
