...
package controllers {
  ...
  class ReverseShoppingCtrl(_prefix: => String) {
    ...
    // @LINE:27
    def cancelOrder(orderId:Long): Call = {
      
      Call("GET", _prefix + { _defaultPrefix } + "cancelOrder/" + play.core.routing.dynamicString(implicitly[play.api.mvc.PathBindable[Long]].unbind("orderId", orderId)))
    }
