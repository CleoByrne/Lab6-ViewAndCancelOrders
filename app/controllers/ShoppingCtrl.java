...
public class ShoppingCtrl extends Controller {
    ...
    public Result viewOrders() {
        List<ShopOrder> order = getCurrentUser().getOrders();
        return ok(viewOrders.render(getCurrentUser(), order));
    }
    ...
    public Result cancelOrder(long orderId){
        ...
        ShopOrder so = ShopOrder.find.byId(orderId);
        Calendar calNow = Calendar.getInstance();

         long miliSecondForDate1 = so.getOrderDate().getTimeInMillis();
         long miliSecondForDate2 = calNow.getTimeInMillis();
     ...
     */
        ...
        if (diffInSecond > 3600) {
            flash("error", "Sorry, But it is to late to cancel your order");
            return ok(viewOrders.render(getCurrentUser(), order));
        }
        else {
            so.cancelledStockReturn();
            so.delete();  
